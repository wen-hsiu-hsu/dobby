...
### Registration & Leave Command Parsing Logic

#### 指令格式說明

- **自己報名/請假**: `@Dobby +1`、`@Dobby 假`
- **代他人操作**: `@Dobby @TargetUser +1`、`@Dobby @TargetUser 假`
- **取消**: `@Dobby -1`、`@Dobby 銷假`

#### Mentionees 處理邏輯 (Registration Parser)

`Registration Parser` 節點負責解析報名與請假指令，並正確識別操作目標（自己或他人）。

**使用 `msg.mention?.mentionees`**:
- LINE Messaging API 的正確屬性名稱（非 `mentions`）
- 參考: `docs/line/2-webhook-events.md`

**跨平台差異問題**:
電腦版的 LINE 似乎無法正確 mention 機器人，導致 `mentionees` 陣列長度不一致：

| 平台 | 指令範例 | `mentionees.length` | 陣列內容 |
|------|---------|---------------------|---------|
| 手機版 | `@Dobby @TargetUser +1` | 2 | `[@Dobby, @TargetUser]` |
| 電腦版 | `@Dobby @TargetUser +1` | 0~1 | `[@TargetUser]`（可能不含 `@Dobby`）|
| 手機版 | `@Dobby +1` | 0~1 | `[@Dobby]` 或為空 |
| 電腦版 | `@Dobby +1` | 0~1 | 為空或 `[@Dobby]` |

**解決方案：一律取最後一個 Mention**

```javascript
const mentions = msg.mention?.mentionees || [];
if (mentions.length > 0 && mentions[mentions.length - 1].type === "user") {
  targetUserId = mentions[mentions.length - 1].userId;
  targetName = cleanTarget;
}
```

**為何取最後一個？**
1. **手機版代操**（2 個 mentionees）：最後一個是目標使用者
2. **電腦版代操**（1 個 mentionee）：最後一個是目標使用者
3. **自己操作**（0 個 mentionees）：不進入 mention 邏輯，走 `isSelf` 分支
4. **指令設計假設**：多個 mention 時，最後一個比較可能是目標對象

**驗證 `type === "user"`**:
- 排除 `type: "all"` 的情況（`@all` 提及整個群組）
- 確保提取的是具體使用者的 `userId`
...
```javascript
{
  command: "+1" | "-1" | "假" | "銷假" | null,
  targetName: string | null,          // 清除 @ 後的名稱（如 "@Vic" → "Vic"）
  targetUserId: string | null,        // 從 mentionees 提取的 LINE userId
  parserError: string | null,         // 解析錯誤訊息
  originalText: string,               // 原始訊息文字
  isSelf: boolean,                    // !targetName && !targetUserId && !parserError
  actorUserId: string                 // event.source.userId（操作者）
}
```

**`isSelf` 判斷邏輯**:
- `true`: 對自己操作（無 `targetName`、無 `targetUserId`、無 `parserError`）
- `false`: 指定他人操作（有 `targetName` 或 `targetUserId`）

#### Resolve Registration Target 分支邏輯

根據 `Registration Parser` 的輸出，`Resolve Registration Target` 節點會判斷三種操作類型：

**Branch 0 (isSelf = true)**：對自己操作
- 輸出:
  - `type`: `"self"`
  - `targetPersonId`: `actorUserId`（操作者的 LINE userId）
  - `targetName`: 從 `Get Actor User` 取得的 `Custom Name` 或 `user_id`
  - `isAdmin`: 操作者的管理員狀態

**Branch 1 (targetUserId 存在)**：透過 @mention 指定目標
- 輸出:
  - `type`: `"mention"`
  - `targetUserId`: `Registration Parser` 的 `targetUserId`
  - `isAdmin`: 操作者的管理員狀態
- 後續流程: 透過 `targetUserId` 查詢 Notion People List

**Branch 2 (targetName 存在)**：透過文字名稱指定目標
- 輸出:
  - `type`: `"name"`
  - `targetName`: `Registration Parser` 的 `targetName`
  - `isAdmin`: 操作者的管理員狀態
- 後續流程: 透過 `targetName` 查詢 USERS database 的 `Custom Name` 欄位

**節點流程**:
```
Registration Parser
  ↓
Get Actor User (查詢 USERS database)
  ↓
Check if isSelf (If 節點)
  ├─ True → Resolve Registration Target (Branch 0)
  └─ False → Get Target User by name (查詢 USERS database)
           → Resolve Registration Target (Branch 1 或 Branch 2)
```
