...
- **流程表模式**：把同一個 `reqId`（同一次 LINE 事件的完整處理過程）收合成一組，由新到舊排列，組內依時間順序顯示「事件進來 → 中間的 API 呼叫 → LINE 回覆」。要看「這次使用者的指令實際觸發了哪些步驟」用這個，不用自己在平面模式裡手動比對時間。每一列左側都有跟平面模式一致的等級色條（綠 info、藍 debug、黃 warn、紅 error），不用點開也能看出哪一步是異常訊息；點擊「終點」（LINE 回覆）那一列也能展開看完整訊息內容/失敗原因，跟平面模式看得到的細節一樣多。

兩種模式的完整內容都是伺服器端一次算好、跟著頁面一起送出，切換模式只是顯示/隱藏其中一個已經渲染好的區塊，不需要等瀏覽器重新整理或重新計算。
...
不屬於 Notion API 呼叫或 LINE 回覆/推播的 log（例如 `handleMessage`、`Routing command`、`Registration updated`、`Leave status updated`、`Event processed` 這類事件/業務摘要 log），不需要程式碼另外處理就能在畫面上看到內容：該筆 log 除了 `level`/`time`/`msg`/`reqId`/`pid`/`hostname` 這幾個後設欄位以外的其餘欄位，會自動顯示成一排小標籤緊接在訊息名稱後面（例如 `handleMessage` 後面會接著 `userId`/`text`/`isAdmin`/`sourceType` 幾個標籤），不用點開就看得到，滑鼠 hover 在標籤上可以看到沒有被截斷的完整值。

這代表**之後新增一種 log 訊息或幫既有 log 多加一個欄位，`/logs` 頁面完全不用改**——只要程式碼裡呼叫 `logger.info({...})`/`logger.debug({...})`，畫面上就會自動有對應的標籤，不需要像 Notion 呼叫/LINE 收發那樣額外寫一份專屬的顯示邏輯。標籤只是單純把值印出來，不會做欄位語意上的解讀（例如布林值就是印 `true`/`false`，物件會印成 JSON 字串），所以想要更精緻的呈現方式（例如自訂 icon、狀態顏色）還是要走 Notion 呼叫/LINE 收發那種專屬顯示，不是這個通用機制的目標。
...
Notion API 呼叫的方法/資料庫徽章旁邊會顯示實際打的 path。這對 `GET /pages/{id}` 這類呼叫特別重要——這種 path 裡不含任何資料庫 ID（頁面 ID 跟資料庫 ID 是兩回事），所以猜不出資料庫名稱、不會有資料庫徽章，path 是唯一能看出「這次到底打了哪個 endpoint」的資訊。Path 太長時畫面上會截斷（超過約 320px 寬），完整內容可以滑鼠 hover 看，或展開該列的完整明細；LINE 回覆/推播的內容預覽也是同樣邏輯，行內顯示到 160 字元、其餘一樣靠 hover 或展開查看。
