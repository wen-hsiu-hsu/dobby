...
export async function handleLeave(
  ...
  const isCurrentlyAbsent = calEvent.absentees.includes(resolved.personPageId);
  ...
  try {
    let newAbsentees: string[];
    if (!isCancel) {
      newAbsentees = [...calEvent.absentees, resolved.personPageId];
    } else {
      newAbsentees = calEvent.absentees.filter((id) => id !== resolved.personPageId);
    }

    await calendarRepo.updateAbsentees(calEvent.pageId, newAbsentees);
