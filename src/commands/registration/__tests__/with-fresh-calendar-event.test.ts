...
beforeEach(() => {
  vi.resetAllMocks();
  vi.mocked(calendarRepo.findByDate).mockResolvedValue(calEvent);
  vi.mocked(mutex.withMutex).mockImplementation(async (_key, fn) => fn());
});

describe('withFreshCalendarEvent', () => {
  it('replies with "not found" when the event does not exist, without locking', async () => {
    vi.mocked(calendarRepo.findByDate).mockResolvedValue(null);
    const mutation = vi.fn();

    await withFreshCalendarEvent('token', 'dobby', '2026-05-09', 'ctx', () => Promise.resolve(calEvent), mutation);
  ...
  it('locks using the event pageId and runs mutation with the refetched value', async () => {
  ...
  it('replies with a generic error and logs with the given context when refetch returns null', async () => {
    await withFreshCalendarEvent('token', 'dobby', '2026-05-09', 'my-context', () => Promise.resolve(null), vi.fn());
