...
import * as calendarMutex from '../calendar-mutex.js';
...
beforeEach(() => {
  ...
  vi.mocked(calendarMutex.withCalendarMutex).mockImplementation(async (_pageId, fn) => fn());
});
...
describe('handleLeave', () => {
  ...
  it('wraps the read-modify-write in withCalendarMutex', async () => {
    await handleLeave(event, false, 'dobby');

    expect(calendarMutex.withCalendarMutex).toHaveBeenCalledWith('evt-1', expect.any(Function));
